...
public class Helper {
	...
	static void menuChoice(int numMenu) throws IOException, ClassNotFoundException {
		switch (numMenu) {
		case 1 : 
			accounts = Helper_Account.starter();
			size = accounts.length;
			break;
		case 2 : 
			if(size == 0) System.out.println("Enter data first.");
			else Helper_Account.showAccounts(accounts);
			break;
		case 3 :
			if(size == 0) System.out.println("Enter data first.");
			else clearAccounts();
	...
	static void encoder(Account[] account) throws FileNotFoundException {
	...
	static Account[] decoder() throws FileNotFoundException {
